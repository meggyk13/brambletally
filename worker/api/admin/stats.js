...
export async function onRequestGet(context) {
  ...
  const row = await context.env.DB.prepare(
    `SELECT
       ...
       (SELECT COUNT(*) FROM users WHERE id != ?1 AND plan = 'supporter') AS users_supporter,
       ...
       (SELECT COUNT(*) FROM users WHERE id != ?1 AND board_blocked_at IS NOT NULL) AS users_board_blocked,
